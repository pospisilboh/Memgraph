...
| :---        |    :----   |
| text_util.levenshteinSimilarity(text1, text2) | Calculate the similarity (a value within 0 and 1) between two texts based on [Levenshtein distance](https://en.wikipedia.org/wiki/Levenshtein_distance). |
| text_util.jaroDistance(text1, text2) | Compute [Jaro](https://en.wikipedia.org/wiki/Jaro%E2%80%93Winkler_distance) string similarity metric of two strings. The Jaro string similarity metric is intended for short strings like personal last names. It is 0 for completely different strings and 1 for identical strings. |
| text_util.jaroWinklerDistance(text1, text2) | The [Jaro-Winkler](https://en.wikipedia.org/wiki/Jaro%E2%80%93Winkler_distance) string similarity metric is a modification of Jaro metric giving more weight to common prefix, as spelling mistakes are more likely to occur near ends of words. The prefix weight is inverse value of common prefix length sufficient to consider the strings *identical*. If no prefix weight is specified, 1/10 is used. |
...
> In Memgraph there is a function [uuid_generator.get()](https://memgraph.com/docs/mage/query-modules/cpp/uuid-generator). The underlying implementation makes use of the uuid-dev library. When using the uuid module on Linux systems, the library can be installed by running sudo apt-get install uuid-dev.
...
Query procedures that allow the users to gain more insight into other query modules and their procedures are written under our utility [mg](https://memgraph.com/docs/mage/usage/loading-modules) query module. This module offers following procedures:
